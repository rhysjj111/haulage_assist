"""Update payslip constraint so that one per week is allowed. Remove ai tables.

Revision ID: 6c1bf900d4d9
Revises: 615139181c4d
Create Date: 2024-12-31 13:01:05.087179
...
def upgrade():
    ...
    with op.batch_alter_table('payslip', schema=None) as batch_op:
        ...
        batch_op.create_unique_constraint('_driver_week_payslip_uc', ['driver_id', sa.func.strftime('%W', sa.column('date'))])
...
def downgrade():
    ...
    sa.Column('id', sa.INTEGER(), server_default=sa.text("nextval('ai_raw_response_id_seq'::regclass)"), autoincrement=True, nullable=False),
    sa.Column('timestamp', postgresql.TIMESTAMP(), server_default=sa.text('now()'), autoincrement=False, nullable=True),
