"""Add Anomaly table to verification models. Remove week_number computed field from Day, Fuel.

Revision ID: 92116be92347
Revises: c477a7b98afa
Create Date: 2025-02-14 06:42:46.482105
...
def upgrade():
    ...
    sa.Column('is_unresolved', sa.Boolean(), nullable=False),
    ...
    with op.batch_alter_table('anomaly', schema=None) as batch_op:
        batch_op.create_index(batch_op.f('ix_anomaly_date'), ['date'], unique=False)
        batch_op.create_index(batch_op.f('ix_anomaly_driver_id'), ['driver_id'], unique=False)
        batch_op.create_index(batch_op.f('ix_anomaly_truck_id'), ['truck_id'], unique=False)
...
def downgrade():
    ...
    with op.batch_alter_table('anomaly', schema=None) as batch_op:
        batch_op.drop_index(batch_op.f('ix_anomaly_truck_id'))
        batch_op.drop_index(batch_op.f('ix_anomaly_driver_id'))
        batch_op.drop_index(batch_op.f('ix_anomaly_date'))
