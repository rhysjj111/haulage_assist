...
class GeminiVerifier(Gemini):
    ...
    def llm_detect_anomalous_mileage(self):
        ...
        formatted_llm_response_collection = []
        for truck_id in range(1,4):
            ...
            query_to_dict(
                ...
                relevant_attributes=['date', 'truck_id', 'start_mileage', 'end_mileage'])

            prompt = f"""
            Act as a data analyst/engineer presenting data for a small haulage company.
            Keep your answer short and data centric.
            The day_data represents days from a driver log, and contains their start and end mileage.
            ...
            1. Create a list of entries that are in ascending order of date.

            2. For each entry (except the last), check that the end_mileage is the same as the start_mileage of the next day.

            3. Create a list of anomalous dates.
            (When labeling the anomalous dates, please indicate the truck id. ie. 'Truck_1 Anomalous Dates: [date1, date2, ...]')

            4. Attach a brief explanation, in as few words as possible, to why the each date is anomalous.

            """
            llm_response = self.generate_response(prompt,0)

            print(f"LLM RESPONSE {truck_id}", llm_response)
