"""Create Anomaly, MissingEntryAnomaly

Revision ID: a8c166ef389c
Revises: c477a7b98afa
Create Date: 2025-02-20 09:24:10.399399
...
def upgrade():
    ...
    op.execute('DROP TYPE IF EXISTS tablename CASCADE')
    ...
    op.create_table('missing_entry_anomaly',
    sa.Column('id', sa.Integer(), nullable=False),
    ...
    with op.batch_alter_table('missing_entry_anomaly', schema=None) as batch_op:
        batch_op.create_index(batch_op.f('ix_missing_entry_anomaly_date'), ['date'], unique=False)
        batch_op.create_index(batch_op.f('ix_missing_entry_anomaly_day_id'), ['day_id'], unique=False)
        batch_op.create_index(batch_op.f('ix_missing_entry_anomaly_driver_id'), ['driver_id'], unique=False)
        batch_op.create_index(batch_op.f('ix_missing_entry_anomaly_truck_id'), ['truck_id'], unique=False)
...
def downgrade():
    ...
    with op.batch_alter_table('missing_entry_anomaly', schema=None) as batch_op:
        batch_op.drop_index(batch_op.f('ix_missing_entry_anomaly_truck_id'))
        batch_op.drop_index(batch_op.f('ix_missing_entry_anomaly_driver_id'))
        batch_op.drop_index(batch_op.f('ix_missing_entry_anomaly_day_id'))
        batch_op.drop_index(batch_op.f('ix_missing_entry_anomaly_date'))

    op.drop_table('missing_entry_anomaly')
